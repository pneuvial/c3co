...
lambda.grid <-  seq(from=1e-6, to=1e-5, length=5)
nb.arch <- 2:6
parameters.grid <- list(lambda1=lambda.grid, lambda2=lambda.grid, nb.arch=nb.arch) 
casRes <- c3co(dat,parameters.grid)
casResTCN <- c3co(dat,parameters.grid, stat="TCN")
```
...
Zplot(df.CHRC1, ylab="C1", ylim=c(0,1))
Zplot(df.CHRC2, ylab="C2", ylim=c(1,3))
```
