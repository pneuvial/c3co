...
lambda.grid <-  seq(from=1e-6, to=1e-5, length=3) ## only of length 3 so that the vignette runs fast enough
nb.arch <- 2:5
...
resC <- c3co(datList, parameters.grid, stat="TCN", verbose=TRUE)
```
...
```{r}
pvePlot(resC@fit, ylim=c(0.80,1))
```
...
result.pveTCN <- sapply(resC@fit, function(cc) cc@PVE)
result.pve <- sapply(res@fit, function(cc) cc@PVE)
##idxBestC1C2 <- min(which(diff(abs(result.pve))<1e-3))+1
best <- 2
...
Wplot(res, idxBest = best)
```
```{r heatmap_TCN, fig.width=7, fig.height=5}
Wplot(resC, idxBest = best)
```
...
df <- createZdf(res, minMaxPos, chromosomes=1, idxBest = best)
Zplot(df)
```
