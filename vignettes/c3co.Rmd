...
W = rSparseWeightMatrix(nb.samp=10, nb.arch=3, sparse.coeff = 0.4)
...
lambda.grid <-  seq(from=1e-6, to=1e-4, length=5) ## only of length 3 so that the vignette runs fast enough
nb.arch <- 2:5
parameters.grid <- list(lambda=lambda.grid, nb.arch=nb.arch) 
