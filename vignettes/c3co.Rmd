...
res <- c3co(datList, parameters.grid)
resC <- c3co(datList, parameters.grid, stat="TCN")
```
...
idxBestTCN <- min(which(diff(abs(result.pveTCN))<1e-2))+1
...
Wplot(casRes, idxBest = 2)
```
```{r heatmap_TCN, fig.width=7, fig.height=5}
Wplot(casResTCN, idxBest = 2)
```
...
df <- createZdf(casRes, minMaxPos, chromosomes=1, idxBest = idxBestC1C2)
Zplot(df)
```
