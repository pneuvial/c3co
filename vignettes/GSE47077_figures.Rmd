...
dataSet <- "GSE47077"
patientID <- "RK29"
data("resC3CO", package = "c3co.data")
```
## Plot PVE
...
Wplot(resC3CO, idxBest=bestp, rownamesW=sprintf("%s-%s",patientID, 1:nrow(dataBest@W)), cexCol = 0.6)
```
...
getAlt <- getAlteredSegments(tol=0.1, dataBest@S$Z1, dataBest@S$Z2, 1, 1.1)
colnames(getAlt) <- sprintf("Subclone %s", letters[1:ncol(dataBest@W)])
df.Alt <- melt(getAlt)
ggplot(df.Alt)+geom_raster(aes(x=factor(X1), y=factor(X2), fill=factor(value)))+ scale_fill_brewer(palette="Set1",name= "Alterations", labels = c("no", "loss", "gain", "loh"))+geom_vline(xintercept=cumsum(sapply(resC3CO@bkp, length)-1))
