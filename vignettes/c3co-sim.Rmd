---
title: "Cancer Subclones from Parent-Specific Copy Numbers"
...
vignette: >
  ...
  %\VignetteIndexEntry{Cancer Subclones from Parent-Specific Copy Numbers}
...
```{r}
datSubClone <- buildSubclones(len, nbClones, bkps, regions, eps = 0.25)
```
...
heatmap.3(cbind(W, 1-rowSums(as.matrix(W))), dendrogram = "row", main = "TRUE",
...
getAlteredSegments <- function(tol, zz1, zz2, c1Mean, c2Mean){
  ...
  ## coding gains as 3
  gain <- apply(gain, MARGIN = 2, FUN = function(gg) {
    idx <- which(gg)
    gg[idx] <- 3
    gg
  })
...
getAlt <- getAlteredSegments(tol = 0.1, dataBest@S$Z1, dataBest@S$Z2, 0.6, 0.8)
