...
###### altered loci determination
      ...
      if(stat=="C1C2"){
        ...
        SESP <- SESPTCN(YTCNhatFull,alteredLociY,ind=1:ncol(alteredLociY), tol, 2)
      }     
...
####################################
rocArrayYFull <- array(dim=c(100, 2, 2,length(tol)),dimnames=list(b=1:100, method=sprintf("%s",stats[2:3]), ROC=c("tp", "fp")))
pathWeight <- Arguments$getWritablePath(sprintf("weightData"))
...
for(b in 1:100){
  ...
  for(ss in 2:3){
    ...
    resSegmentation <-  readRDS(sprintf("%s/segDat.rds",pathMeth))
    Y <- resSegmentation$Y
    ...
    if(stat=="C1C2"){
      ...
      YhatFull <- expand(Y1+Y2,start, end)
    ...
    rocArrayYFull[b,stat,"tp",] <- unlist(SESP["tp",])
    rocArrayYFull[b,stat,"fp",] <- unlist(SESP["fp",])   
  }
}
...
gAUC <- ggplot(df.auc.tot, aes(x=method, y=AUC))+geom_boxplot(aes( fill=method))+theme_bw()+geom_hline(data=df.aucY, aes(yintercept=value, lty=Y),show_guide = TRUE)+facet_grid(.~stat)
gAUC
pathFig <- Arguments$getWritablePath("Figures/AUC")
ggsave(gAUC, filename=file.path(pathFig, "AUCs_n=24000.pdf"), width=10, height=7)
...
randIndexArrayY <- array(dim=c(100, 1),dimnames=list(b=1:100, method="Y"))
pathWeight <- Arguments$getWritablePath(sprintf("weightData"))
pathSim <- Arguments$getWritablePath(sprintf("simData"))
for(b in 1:100){
  print(b)
  ...
  tcnB <- t(sapply(dat, function(dd) dd$tcn))
  clustY <- cutree(hclust(dist(tcnB),method="ward.D"), ncol(WT))
  randIndex <- adjustedRandIndex(clustWT, clustW)
  randIndexArrayY[b] <-  adjustedRandIndex(clustWT, clustY)
}

dataArrayrandIndex <- data.frame(melt(randIndexArray))
gRandIndex <- ggplot(dataArrayrandIndex)+geom_boxplot(aes(x=method, y=value,fill=method))+ylab("Rand Index")+xlab("")+theme_bw()+geom_hline(yintercept=mean(randIndexArrayY),show_guide = TRUE)
gRandIndex
...
if(!file.exists(file.path(rocDataPath, fileROC))||forceROC){
  for(b in 1:100){
    ...
    M <- readRDS(file.path(pathWeight, filename))
    WT <- cbind(M, 100-rowSums(M))/100
    composition <- apply(WT, 1, function(mm) {
      ww <- which(mm!=0)
      return(ww)
    })
    ...
    for(ss in 1:length(stats)){
      ...
      ind <- apply(corrBestW, 2, which.max)
      compositionWhat <- apply(W, 1, function(mm) {
        ww <- which(mm!=0)
        return(ww)
      })
      ...
      if(stat=="C1C2"){
        ...
        alteredLociZ1Z2 <- sapply(1:nrow(Z1hatFull), function(ii) {
          getPosC1C2( tt=tt, c1Mean=1,c2Mean=1, zz1=Z1hatFull[ii,],zz2=Z2hatFull[ii,])
        }   )

        alteredLociYhat <- sapply(compositionWhat, function(cc) rowSums(alteredLociZ1Z2[,cc]))
        alteredLociYhat <- apply(alteredLociYhat, 2, function(cc){
          wwA <- which(cc!=0)
          ww <- which(cc==0)
          alt <- logical(0)
          alt[wwA] <- TRUE
        ...
        tpfp <- sapply(1:nrow(alteredLociYhat), function(llhat){
          sapply(1:nrow(alteredLociY), function(llobs){
            tp <- sum(llobs==llhat)
            pos <- sum(llobs)
            fp <- length(which(llobs==FALSE & llhat==TRUE))
            neg <- length(llobs)-sum(llobs)
            return(list(tp=tp, fp=fp, neg=neg, pos=pos))
          })       
        })
...
randIndexArrayFull <- cbind(randIndexArray, randIndexArrayY)
names(randIndexArrayFull) <- c("b", "method")
dataArrayrandIndex <- data.frame(melt(randIndexArrayFull))
dataArrayrandIndex$method <- dataArrayrandIndex$X2
