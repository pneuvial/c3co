...
casRes <- c3co(dat,lambda1.grid, lambda2.grid, nb.arch.grid = 2:6, output.dir="result_c3co_C1C2")
casResTCN <- c3co(dat,lambda1.grid, lambda2.grid, nb.arch.grid = 2:6, stat="TCN", output.dir="result_c3co_TCN")
```
...
Wplot(casRes[[idxBestC1C2]]@res)
```
```{r heatmap_TCN, fig.width=7, fig.height=5}
Wplot(casResTCN[[idxBestTCN]]@res)
```
...
dataBest <- casRes[[idxBestC1C2]]
minMaxPos <- data.frame(chr=1, minPos=1, maxPos=len)
df.CHR <- createZdf(dataBest, minMaxPos, chromosomes=1, var="TCN")
df.CHRC1 <- createZdf(dataBest, minMaxPos, chromosomes=1, var="Minor")
df.CHRC2 <- createZdf(dataBest, minMaxPos, chromosomes=1, var="Major")
