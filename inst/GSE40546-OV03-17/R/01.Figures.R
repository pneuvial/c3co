...
gArchC1 <- ggplot(df.CHRC1)+geom_step(aes(bkp, Z, group=arch, col=arch,lty=arch), direction="hv", lwd=2)+facet_wrap(~chr, ncol=2, scale="free")+theme_bw()
gArchC1
...
gArchC2 <- ggplot(df.CHRC2)+geom_step(aes(bkp, Z, group=arch, col=arch,lty=arch), direction="hv", lwd=2)+facet_wrap(~chr, ncol=2, scale="free")+theme_bw()
gArchC2
